...
function PostBody() {
    ...
    const handleLikeClick = async (postId) => {
        ...
        axios.post(`${process.env.REACT_APP_BACKEND_URL}/addLike/${postId}`,
            { isLiked: likeStatus, token })
            .then((res) => {
                console.log(res);
            })
            .catch((err) => {
    ...
    const submitCommentHandler = (event) => {
        ...
        axios.post(`${process.env.REACT_APP_BACKEND_URL}/createComment/${postId}`,
            { commentBody, token })
            .then((res) => {
                console.log(res);
            })
    ...
    function sendPost(token) {
        ...
        axios.post(`${process.env.REACT_APP_BACKEND_URL}/getLoggedinUser`, { token })
            .then((res) => {
                // console.log(res);
                setLoggedinUser(res.data.user);

                getAllPosts(res.data.user);
        ...
        function getAllPosts(user) {
            ...
            axios.post(`${process.env.REACT_APP_BACKEND_URL}/getAllPosts`, { token })
                .then((res) => {

                    res.data.posts.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

                    //filters the other private posts of other users
                    res.data.posts = res.data.posts.filter((post) => (
                        !post.isPrivate || String(post.owner._id) === String(user._id)
                    ));
                ...
                <>
                    ...
                    <div class="_feed_inner_timeline_content _padd_r24 _padd_l24">
                        <div class="_feed_inner_timeline_post_top">
                            <div class="_feed_inner_timeline_post_box">
                                ...
                                <div class="_feed_innconsole.log(res.data.posts);er_timeline_post_box_txt">
                                    <h4 class="_feed_inner_timeline_post_box_title">{post.owner.firstname} {post.owner.lastname}</h4>
                                    <p class="_feed_inner_timeline_post_box_para">
                                        {new Date(post.createdAt).toLocaleDateString()} &nbsp;
                                        {new Date(post.createdAt).toLocaleTimeString()} &nbsp;
                                        <a href="#0">{post.isPrivate === true ? "Private" : "Public"}</a>
                        ...
                        <h4 class="_feed_inner_timeline_post_title">{post.description}</h4>
                    ...
                    <div class="_feed_inner_timeline_total_reacts _padd_r24 _padd_l24 _mar_b26">
                        ...
                        <div class="_feed_inner_timeline_total_reacts_txt">
                            ...
                            <p class="_feed_inner_timeline_total_reacts_para2"><span></span> {post.likes.length}&nbsp; Likes</p>
                        </div>
                    </div>
                    <div class="_feed_inner_timeline_reaction"

                    >
                        <button class="_feed_inner_timeline_reaction_emoji _feed_reaction "
                            ...
                            >
                                <div class="_feed_inner_comment_box_content">
                                    ...
                                    <div class="_feed_inner_comment_box_content_txt">
                                        <textarea class="form-control _comment_textarea" placeholder="Write a comment" id="floatingTextarea2"
                                            value={commentBody}
                                ...
                                <div class="_feed_inner_comment_box_icon">
                                    <button type="submit"
                                        ...
                                        style={{ backgroundColor: "#377DFF", borderRadius: "11px", border: 'none', color: "white", height: "2rem", width: "4rem" }}
                                    > Post
                                    ...
                                    <div class="_comment_details">
                                        <div class="_comment_details_top">
                                            <div class="_comment_name">
                                                <a href="profile.html ">
                                                    <h4 class="_comment_name_title">{comment.userId.firstname}&nbsp;{comment.userId.lastname}</h4>
                                                </a>
                                            </div>
                                        </div>
                                        <div class="_comment_status" >
                                            <p class="_comment_status_text" style={{ margin: "10px 60px" }}>
                                                <span>
                                                    {comment.commentBody}
                                                </span>
                                            </p>
